...
echo "<div class='text-center'>";
		...
		$tourId=$this->media->getTourId();
		$tour=TourRepository::getById($tourId);
        $tours=TourRepository::getAll(0,2000);
		//Update media 
		if (PRODUCTION==1) echo "<form method='post' action='".PATHSERVERSININDEX."Media/updateVirtualTour' >";
		else echo "<form method='post' action='".PATHSERVER."Media/updateVirtualTour' >";
			?>
			<div class='form-group m-4' >  
				<label for='name' class='control-label'>Name:</label> 
				<div class='col'>
					<?php
					if(isset($this->errors)) echo "<input type='text' class='form-control' name='name' id='name' title='name' value='".$this->name."' />";
					else echo "<input type='text' class='form-control' name='name' id='name' title='name' value='".$this->media->getName()."' />";
					?>
				</div>
			</div> 
			...
			<input type="hidden" name="id" id="id" value="<?php echo $this->media->getId();?>" />
	...
	<div class="modal-dialog">
		...
		<div class="modal-footer">
			...
			<form method="post" action='<?php echo PATHSERVER; ?>Media/deleteVirtualTOur ?>' >
