...
$total_registers=0;
function get_count_competitions( $sqlite, $where_name="",$where_value=""){
    $rowCount=0;
    try {
        if(empty( $where_name)){
            $result =  $sqlite->query("SELECT count(*) FROM competitions;");
        }else{
            $result =  $sqlite->query("SELECT count(*) FROM competitions WHERE {$where_name} LIKE '%{$where_value}%';"); 
        }
...
function get_competitions($sqlite, $init=0, $where_name="",$where_value="", $order="competicion_id"){
    $competitions=[];
    try {
        // Consultar los datos de la tabla
        if(empty( $where_name)){
            $result =  $sqlite->query("SELECT * FROM competitions ORDER BY {$order} LIMIT {$init}, 10;");
        }else{
            $result =  $sqlite->query("SELECT * FROM competitions WHERE {$where_name} LIKE '%{$where_value}%' ORDER BY {$order} LIMIT {$init}, 10;");
        }
        while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
            $competitions[] = $row;
        }
...
if(isset($_POST['submit'])){
    ...
    $competitions=get_competitions($sqlite,$page*10, $_POST['where_name'],$_POST['where_value']);
    $total_registers= get_count_competitions($sqlite, $_POST['where_name'],$_POST['where_value']);
}else{
    //Si no se ha recibido nada del formulariod e configuración pero se ha recibido al por la URL
    if(isset($_GET['where_name'])){
        if (isset($_GET['order_by'])) {
            $competitions=get_competitions($sqlite,$_GET['page']*10, $_GET['where_name'],$_GET['where_value'], $_GET['order_by']);
            $total_registers= get_count_competitions($sqlite, $_GET['where_name'],$_GET['where_value'], $_GET['order_by']);
        } else {
            $competitions=get_competitions($sqlite,$_GET['page']*10, $_GET['where_name'],$_GET['where_value']);
            $total_registers= get_count_competitions($sqlite, $_GET['where_name'],$_GET['where_value']);
        }
    }else{
        $competitions=get_competitions($sqlite);
        $total_registers= get_count_competitions($sqlite);
    }
}
$sqlite->close();
?>
