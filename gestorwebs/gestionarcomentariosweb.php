...
<?php
	...
	function dibujarLayoutComentarios(){
		echo "<div class='divcomentariosweb'>";
			echo "<h3>Comentarios  de webs no validados<br /></h3>";
			$basededatos= new Mysql();
			$basededatos->conectar_mysql();
			$consulta= "SELECT * FROM comentariosweb WHERE validadocomentarioweb=0 ORDER BY fechacomentarioweb";
			$resultado=$basededatos->ejecutar_sql($consulta);
			echo "<ul style='list-style:none'>";
					while ($linea = mysqli_fetch_array($resultado)) 
					{
							echo "<a href=gestionarcomentariosweb.php?accion=2&codigoValidacion=$linea[idcomentarioweb]> Identificador: ".$linea['idcomentarioweb'].", nombre: ".$linea['nombrecomentarioweb'].", texto: ".$linea['textocomentarioweb']."</a><br />";
					}
	...
	function validarUnComentarioWeb($codigoValidacion, $idWeb){
		echo "<div class='divcomentariosweb'>";
			//echo "<h3>Pincha Si o No<br /></h3>";
			$basededatos= new Mysql();
			$basededatos->conectar_mysql();
			$consulta= "SELECT * FROM comentariosweb WHERE validadocomentarioweb=0 && idcomentarioweb='".$codigoValidacion."' ORDER BY fechacomentarioweb";
			$resultado=$basededatos->ejecutar_sql($consulta);
			if(mysqli_num_rows($resultado)==0){
				 echo "Este comentario de una web ya ha sido validado, borra el mensaje de correo.";
			}
			echo "<ul style='list-style:none'>";
					while ($linea = mysqli_fetch_array($resultado)) 
					{
							echo "<p>Identificador: ".$linea['idcomentarioweb']."</p><p>Nombre: ".$linea['nombrecomentarioweb']."</p><p>Texto: ".$linea['textocomentarioweb']."</p>";
							echo "<p><a href=gestionarcomentariosweb.php?idComentarioWeb=$codigoValidacion&accion=3&idWeb=$idWeb>SI para grabarlo</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p><a href=gestionarcomentariosweb.php?idComentarioWeb=$codigoValidacion&accion=4&idWeb=$idWeb>NO para borrarlo definitivamente</a>";
					}
					$basededatos->desconectar();
			echo "</ul>";
		echo "</div>";
	...
	if (!isset($_GET['accion'])){
		dibujarLayoutComentarios();
	}
	if($_GET['accion']==2){
		validarUnComentarioWeb($_GET['codigoValidacion'], $_GET['idWeb']);
	}else if($_GET['accion']==3){
		$mensaje="Comentario validado!!!";
		$bd= new Mysql();
		$bd->conectar_mysql();
		$sql="update comentariosweb set validadocomentarioweb=1 where idcomentarioweb='".$_GET[idComentarioWeb]."'";
		$bd->ejecutar_sql($sql);
		$bd->desconectar();
		echo "<script type='text/javascript'>location.href='gestionarwebs.php?accion=1&idWeb=$_GET[idWeb]&mensaje=$mensaje'</script>";
	}else if($_GET['accion']==4){
		$mensaje="Mensaje de la web borrado.";
		$bd= new Mysql();
		$bd->conectar_mysql();
		$sql="DELETE FROM comentariosweb WHERE idcomentarioweb='".$_GET[idComentarioWeb]."' LIMIT 1";
		$bd->ejecutar_sql($sql);
		$bd->desconectar();
		echo "<script type='text/javascript'>location.href='gestionarwebs.php?accion=1&idWeb=$_GET[idWeb]&mensaje=$mensaje'</script>";
	}
